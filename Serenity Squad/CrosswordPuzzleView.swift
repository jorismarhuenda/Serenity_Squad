...
struct CrosswordPuzzleView: View {
    @State private var grid: [[CrosswordCell]] = Array(repeating: Array(repeating: CrosswordCell(letter: " ", isClue: false), count: 10), count: 10)
        ...
        VStack {
            ...
            CrosswordGridStack(rows: 10, columns: 10) { row, col in
                CrosswordCellView(cell: self.grid[row][col])
                    .background(self.selectedCell?.row == row && self.selectedCell?.col == col ? Color.yellow : Color.white)
                    .onTapGesture {
    ...
    func startNewGame() {
        grid = Array(repeating: Array(repeating: CrosswordCell(letter: " ", isClue: false), count: 10), count: 10)
        selectedCell = nil
        ...
        let wordList = [
            ...
            // Ajoutez plus de mots pour remplir davantage la grille
        ...
        for _ in 0..<100 {
            ...
            if canPlaceWord(word: letters, at: (row, col), direction: direction) {
                for (i, letter) in letters.enumerated() {
                    ...
                    grid[r][c] = CrosswordCell(letter: i == 0 ? letter : " ", isClue: true, number: i == 0 ? index + 1 : nil)
                }
    ...
    func canPlaceWord(word: [Character], at start: (row: Int, col: Int), direction: Direction) -> Bool {
        for (i, _) in word.enumerated() {
            ...
            if r >= 10 || c >= 10 || (grid[r][c].isClue && grid[r][c].letter != " ") {
                return false
            }
        }
        return true
...
struct CrosswordCellView: View {
    ...
    var body: some View {
        ZStack {
            Rectangle()
                .foregroundColor(cell.isClue ? Color.white : Color.gray.opacity(0.5))
